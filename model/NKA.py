...
class NKA:
    ...
    def __str__(self):
        ...
        for symb in self.alphabet:
            text += str(symb) + ', '
        text = text[:-2] + '\n'
        for ind, cond_id in enumerate(self.nka_transitions):
    ...
    def perform_act(self, sub_reg: str, start_cond_id: int) -> int:
        ...
        while True:
            ...
            if sub_reg[cur_symb_ind] in self.alphabet:
                ...
                self.nka_transitions_with_e[start_id].cond_transition['^'].append(start_end_conds_ids[0])
    ...
    def choice_action(self, sub_reg: str) -> tuple[int, int]:
        ...
        self.nka_transitions_with_e.append(start_cond)
        self.nka_transitions_with_e.append(end_cond)
    ...
    def sequence_action(self, symb: str) -> tuple[int, int]:
        ...
        self.nka_transitions_with_e.append(start_cond)
        self.nka_transitions_with_e.append(end_cond)
        self.nka_transitions_with_e[start_cond.id].cond_transition[symb].append(end_cond.id)
        return start_cond.id, end_cond.id
    ...
    def cycle_action(self, sub_reg: str) -> tuple[int, int]:
        ...
        self.nka_transitions_with_e.append(start_cond)
        self.nka_transitions_with_e.append(end_cond)
        self.nka_transitions_with_e.append(cycle_start_cond)
        self.nka_transitions_with_e.append(cycle_end_cond)
        self.nka_transitions_with_e[start_cond.id].cond_transition['^'].append(end_cond.id)
        self.nka_transitions_with_e[start_cond.id].cond_transition['^'].append(cycle_start_cond.id)
        self.nka_transitions_with_e[cycle_end_cond.id].cond_transition['^'].append(cycle_start_cond.id)
        self.nka_transitions_with_e[cycle_end_cond.id].cond_transition['^'].append(end_cond.id)
    ...
    def process_sub_parts(self, start_cond_id: int, end_cond_id: int, sub_reg: str):
        ...
        for reg_part in separated_regular:
            ...
            self.nka_transitions_with_e[conds_ids].cond_transition['^'].append(end_cond_id)
    ...
    def get_e_transitions_path_recurs(self, start_cond: NkaConditionE) -> dict[str: list[int]]:
        ...
        for e_path_ind in start_cond.cond_transition['^']:
            if len(self.nka_transitions_with_e[e_path_ind].through_e_transition.keys()) == 0:
                self.nka_transitions_with_e[e_path_ind].through_e_transition = self.get_e_transitions_path_recurs(self.nka_transitions_with_e[e_path_ind])
                if -1 in self.nka_transitions_with_e[e_path_ind].through_e_transition['^']:
                    self.final_conds_ids.append(e_path_ind)
            e_path_final_dict.append(self.nka_transitions_with_e[e_path_ind].through_e_transition)

        return merge_dicts(e_path_final_dict)

    def create_nka(self):
        for cond in self.nka_transitions_with_e:
            if len(cond.through_e_transition.keys()) == 0:
                self.nka_transitions_with_e[cond.id].through_e_transition = self.get_e_transitions_path_recurs(self.nka_transitions_with_e[cond.id])
                if -1 in self.nka_transitions_with_e[cond.id].through_e_transition['^']:
                    self.final_conds_ids.append(cond.id)

                for key, cond_ids in self.nka_transitions_with_e[cond.id].through_e_transition.items():
                    for cond_id in cond_ids:
                        self.nka_transitions_with_e[cond_id].is_important_cond = True

        # print(self.final_conds_ids)

        self.nka_transitions_with_e[0].is_important_cond = True
        tmp_final_conds: list[int] = []
        for cond in self.nka_transitions_with_e:
            if cond.is_important_cond:
                if cond.id in self.final_conds_ids:
                    tmp_final_conds.append(cond.id)
                self.nka_transitions.append(NkaCondition(cond.id))
                self.nka_transitions[-1].cond_transition = cond.through_e_transition
                self.nka_transitions[-1].cond_transition.pop('^', None)
        # print(tmp_final_conds)
        self.final_conds_ids = tmp_final_conds

        id_dict: dict[int, int] = {}
        for ind, cond in enumerate(self.nka_transitions):
            id_dict[cond.id] = ind

        for cond_ind, cond in enumerate(self.nka_transitions):
            self.nka_transitions[cond_ind].id = id_dict[cond.id]
            for key, vals in cond.cond_transition.items():
                for ind, val in enumerate(vals):
                    self.nka_transitions[cond.id].cond_transition[key][ind] = id_dict[val]
...
def delete_duplicates_in_dict_vals(origin_dict: dict[str:list[int]]):
    changed_dict: dict[str:list[int]] = {}
    for key, vals in origin_dict.items():
        changed_dict[key] = list(set(vals))
        changed_dict[key].sort()
    return changed_dict
